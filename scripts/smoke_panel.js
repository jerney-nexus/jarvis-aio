...
setTimeout(async () => {
  ...
  checks.push(
    ...
    ["floor plan: device pin style reflects entity state", (() => { el._hass.states["light._pin_test"] = { state: "on", attributes: { friendly_name: "Pin Test" } }; const s = el._entMarkerStyle("light._pin_test"); delete el._hass.states["light._pin_test"]; return s && s.name === "Pin Test" && s.val === "ON"; })()],
    ["floor editor: devices-on-plan picker + imported-plan opacity slider", (() => { const h = el._renderFloorPlanEditor(el._data()); return /id="ent-add"/.test(h) && /id="fp-ent-input"/.test(h) && /id="fp-bg-op"/.test(h); })()],
    ["floor plan: entity pins + custom bg opacity render on the SVG", (() => { try { const se = el._editingEntities, sc = el._liveData.config.floor_plan_bg, so = el._liveData.config.floor_plan_bg_opacity; el._editingEntities = { "1f": [{ e: "light._pin_test", x: 60, y: 40 }] }; el._hass.states["light._pin_test"] = { state: "on", attributes: { friendly_name: "Lamp" } }; el._liveData.config.floor_plan_bg = JSON.stringify({ "1f": "data:image/png;base64,AAA" }); el._liveData.config.floor_plan_bg_opacity = "0.75"; const svg = el._renderEditableSVG({ "1f": { rooms: [{ name: "Living", x: 0, y: 0, w: 120, h: 80 }] } }, "1f"); el._editingEntities = se; el._liveData.config.floor_plan_bg = sc; el._liveData.config.floor_plan_bg_opacity = so; delete el._hass.states["light._pin_test"]; return /class="fp-ent"/.test(svg) && /fp-ent-dot/.test(svg) && /opacity="0.75"/.test(svg); } catch (e) { return false; } })()],
    ["settings grid uses CSS grid, not multicol (reliable desktop section toggling)", (() => { const s = el._styles(); const m = s.match(/\.settings-grid\s*\{[^}]*\}/); return !!m && /display:\s*grid/.test(m[0]) && !/column-width/.test(m[0]); })()],
